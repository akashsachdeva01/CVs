...
def search_jobs(request: JobSearchRequest):
    ...
    # 1. Scrape Jobs (Live)
    raw_jobs = scraper.get_jobs(search_keyword, request.location)
    ...
    for job in raw_jobs:
        ...
        # Only return jobs if at least one skill matched
        if score_data["score"] == 0 and len(score_data["matches"]) == 0:
            continue
