...
def get_jobs(target_role: str, location: str, f_jt: str = "", f_e: str = "", f_tpr: str = "r604800"):
    """
    ...
    search_url = f"https://uk.linkedin.com/jobs/search?keywords={query_role}&location={query_loc}&sortBy=DD"
    if f_tpr:
        search_url += f"&f_TPR={f_tpr}"
    if f_jt:
        search_url += f"&f_JT={urllib.parse.quote(f_jt)}"
    if f_e:
        search_url += f"&f_E={urllib.parse.quote(f_e)}"
